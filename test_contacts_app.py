...
def test_create_contact_duplicate_email(client):
    ...
    response = create_contact(client, name="Other Name")
...
def test_update_contact_duplicate_email(client):
    create_contact(client, "A", "a@example.com", "111")
    second = create_contact(client, "B", "b@example.com", "222")
    second_id = second.get_json()["id"]
 
    response = client.put(f"/contacts/{second_id}", json={"email": "a@example.com"})
    assert response.status_code == 409
    assert response.get_json()["error"] == "email already exists"
